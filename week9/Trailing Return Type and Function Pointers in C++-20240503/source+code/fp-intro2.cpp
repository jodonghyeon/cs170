...
// - Defining a function to return a function pointer
// - Passing a function pointer to a function as a callback function
...
*/
int eval_switch(char opcode, int x, int y) {
  switch (opcode) {
    case '+': return Add(x, y);
    case '-': return Sub(x, y);
    case '*': return Mul(x, y);
...
*/
#if 1
int eval_switch_funcptr(int (*FP)(int, int), int a, int b) {
  return FP(a, b);
}
#else
...
*/
int eval_noswitch(char opcode, int x, int y) {
